...
FD3D12RootSignature* FD3D12RootSignatureManager::GetRootSignature(const FD3D12ComputeShader* ComputeShader)
{
	check(ComputeShader);
	auto It = ComputeRootSignatureMap.find(ComputeShader->ShaderHash);

	if (It == ComputeRootSignatureMap.end())
	{
		return CreateRootSignature(ComputeShader);
	}
...
FD3D12RootSignature* FD3D12RootSignatureManager::CreateRootSignature(const FD3D12ComputeShader* ComputeShader)
{
	FD3D12RootSignature* NewRootSignature = new FD3D12RootSignature(GetParentAdapter(), ComputeShader);
	ComputeRootSignatureMap.emplace(ComputeShader->ShaderHash, NewRootSignature);
...
void FD3D12RootSignature::Init(const FD3D12ComputeShader* ComputeShader)
{
	FD3D12RootSignatureDesc Desc(ComputeShader);
	Init(Desc.GetDesc());
	Hash = Desc.Hash;
}
...
FD3D12RootSignatureDesc::FD3D12RootSignatureDesc(const FD3D12ComputeShader* ComputeShader)
{
	uint32 RootParameterCount = 0;

	if (ComputeShader)
	{
		if (ComputeShader->ResourceCounts.NumSRVs > 0)
		{
			DescriptorRanges[RootParameterCount].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, ComputeShader->ResourceCounts.NumSRVs, 0, 0);
		...
		{
			for (uint32 ShaderRegister = 0; (ShaderRegister < ComputeShader->ResourceCounts.NumCBVs) && (ShaderRegister < 16); ++ShaderRegister)
			{
			...
			{
				DescriptorRanges[RootParameterCount].Init(D3D12_DESCRIPTOR_RANGE_TYPE_CBV, ComputeShader->ResourceCounts.NumCBVs - 16, 16, 0);
				TableSlots[RootParameterCount].InitAsDescriptorTable(1, &DescriptorRanges[RootParameterCount], D3D12_SHADER_VISIBILITY_ALL);
				++RootParameterCount;
				RootParameterSize += 1;
		...
		{
			DescriptorRanges[RootParameterCount].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SAMPLER, ComputeShader->ResourceCounts.NumSamplers, 0, 0);
			TableSlots[RootParameterCount].InitAsDescriptorTable(1, &DescriptorRanges[RootParameterCount], D3D12_SHADER_VISIBILITY_ALL);
			++RootParameterCount;
			RootParameterSize += 1;
		...
		{
			DescriptorRanges[RootParameterCount].Init(D3D12_DESCRIPTOR_RANGE_TYPE_UAV, ComputeShader->ResourceCounts.NumUAVs, 0, 0);
			TableSlots[RootParameterCount].InitAsDescriptorTable(1, &DescriptorRanges[RootParameterCount], D3D12_SHADER_VISIBILITY_ALL);
			++RootParameterCount;
			RootParameterSize += 1;
